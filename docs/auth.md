...
logout / reuse-detection, не истекла) — без этой проверки access-token
продолжал бы работать до своего `exp` даже после logout. На последнем
шаге достаёт пользователя по claim `sub`. Любое из следующих условий →
`401 invalid_token`:

- отсутствует заголовок или неверная схема;
- битая подпись;
- просроченный access (`exp <= now`);
- сессия из `sessionId` отозвана (logout, reuse-detection) или истекла;
